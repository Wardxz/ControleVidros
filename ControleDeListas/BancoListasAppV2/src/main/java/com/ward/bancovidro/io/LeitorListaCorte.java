...
public class LeitorListaCorte {
    ...
    public static List<RegistroLista> lerArquivo(File arquivo, String nomeObraManual) {
        ...
        Map<String, Stats> agrupamento = new HashMap<>();   // Mapa para somar vidros iguais
        ...
        try (FileInputStream fis = new FileInputStream(arquivo);
            ...
            while (it.hasNext()) {
                ...
                if (!headerAchado) {
                    for (Cell cell : row) {
                        ...
                        // Procura coluna de identificação (Tipologia, Posição, Código ou Item)
                ...
                try {
                    ...
                    if (qtd > 0 && larg > 0) {  // Agrupa por especificação para somar m ao quadrado
                        // A chave do mapa garante que vidros iguais (mesmo tipo e tamanho) sejam somados
                        // Usando larg/alt na chave para não misturar vidros de tamanhos diferentes
            ...
            for (Stats st : agrupamento.values()) {
                // O LinkedHashSet remove duplicatas automaticamente e mantém ordem
    ...
    static class Stats {
        ...
        // Talvez mudar para List<String> seja melhor
    }
