...
public class GerenciadorVidros {
    ...
    // Funções de Filtro e Controle (CRUD)
    ...
    public void adicionarListaDeObra(List<Vidro> novaLista) {
        ...
        // Será chamada pelo ImportadorExcel
    }
    ...
    // Chegada na fábrica
    public void darEntradaFabrica(String idItemUnico, int quantidade, String nfe) {
        Vidro item = controleGeral.stream()
                .filter(v -> v.getIdItemUnico().equals(idItemUnico))
                .findFirst().orElseThrow(() -> new IllegalArgumentException("Item não encontrado"));
    ...
    // Vidros Cortados
    public void darBaixaCorte(String idItemUnico, int quantidade) {
        Vidro item = controleGeral.stream()
                .filter(v -> v.getIdItemUnico().equals(idItemUnico))
                .findFirst().orElseThrow(() -> new IllegalArgumentException("Item não encontrado"));

        item.setQtdCortada(item.getQtdCortada() + quantidade);
        item.calcularStatus();  // Atualiza statusGeral
        this.salvarDados();
    }
